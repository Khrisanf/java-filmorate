...
public class FilmService {
   ...
   @Transactional
    ...
    public List<Film> search(String query, Set<SearchBy> searchBy) {
        ...
        films.forEach(film -> {
            Set<Director> validDirectors = new HashSet<>();

            for (Director director : film.getDirectors()) {
                try {
                    Director validDirector = directorService.findById(director.getId());
                    validDirectors.add(validDirector);
                } catch (NotFoundException e) {
                    log.debug("Director {} is deleted, filtering out", director.getId());
                }
            }

            film.setDirectors(validDirectors);
        });
